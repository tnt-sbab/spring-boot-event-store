...
interface EventRepository : CrudRepository<EventEntity, Long> {
    ...
    fun findAllAggregateIds(): List<UUID>

    @Query("select distinct aggregateId from EVENTS where occurredAt >= :occurredAt")
    fun findAggregateIdsByOccurredAtAfter(occurredAt: Instant): List<UUID>

    @Query("select distinct aggregateId from EVENTS where occurredAt <= :occurredAt")
    fun findAggregateIdsByOccurredAtBefore(occurredAt: Instant): List<UUID>
}
